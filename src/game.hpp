...
std::vector<Obstacle> obstacle_create() {
  ...
  float obstacle_gap =
      ...
      5; // Takes the gap between two obstacles by taking the width of all the 4
         // obstacles and subtracting it from the screen width and dividing it
         // by 5 cause for 4 obstacles you have to add 5 gaps ( 3 for each other
         // and 2 for the parameters of the screen )
  for (int i = 0; i != 4; i++) {
    float obstacle_position_x = (i + 1) * obstacle_gap + i * obstacle_width;
    obstacles.push_back(
        Obstacle({obstacle_gap, float((GetScreenHeight() - 100))}));
  }
  return obstacles;
}
